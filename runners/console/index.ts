...
export class Console extends Runner {
    ...
    runExecuteFile(runCommand: RunCommand): RunResult {
        ...
        ? "powershell.exe  \"& \" \" .\\" + path.basename(runCommand.command.parameters[0]) + "\""
        : "bash " + path.basename(runCommand.command.parameters[0]);
    ...
    async assertExecuteFile(runCommand: RunCommand, result: RunResult) {
        try {
            let assert = new Assertions()
            .noErrorCode(result)
            .noException(result);
            ...
            if(runCommand.command.parameters.length > 2) {
                if(runCommand.command.parameters[1].asynchonous){
                    if(!runCommand.command.parameters[2].startupTime) {
                        console.warn("No startup time for command executeFile has been set")
                    }
                    let startupTimeInSeconds = runCommand.command.parameters[2].startupTime ? runCommand.command.parameters[2].startupTime : 0;
                    await this.sleep(runCommand.command.parameters[2].startupTime);
                    ...
                    if(!runCommand.command.parameters[2].port) {
                        ...
                        let isReachable = await assert.serverIsReachable(runCommand.command.parameters[2].port, runCommand.command.parameters[2].path);
                        if(!isReachable) {
                            this.killAsyncProcesses();
                            throw new Error("The server has not become reachable in " + startupTimeInSeconds + " seconds: " + "http://localhost:" + runCommand.command.parameters[2].port + "/" + runCommand.command.parameters[2].path)
                        }
                    }
                }
            }
